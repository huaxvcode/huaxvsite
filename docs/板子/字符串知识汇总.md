...
namespace lrq {
    ...
    void init_dv() {
        ...
        for (ll i = 1; i <= n; i ++) {
            for (ll j = i; i * j <= n; j ++) {
                dv_mi[i * j].push_back(i);
                if (i - j) dv_mx[i * j].push_back(j);
            }
        }
    }
    void get_loop(ll& len, ll l, ll r) {
        ...
        auto& dmi = dv_mi[tn];
        auto& dmx = dv_mx[tn];
        for (auto it = dmi.begin(); it != dmi.end(); it ++) {
            if (get(l, r - *it) == get(l + *it, r)) { len = *it; return; }
        }
        for (auto it = dmx.rbegin(); it != dmx.rend(); it ++) {
            if (get(l, r - *it) == get(l + *it, r)) { len = *it; return; }
        }
        // return std::string(s.begin() + 1, s.begin() + 1 + len);
    }
