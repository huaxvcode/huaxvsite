...
- [字符串知识汇总](#字符串知识汇总)
  ...
  - [字符串哈希 - 求最小循环节](#字符串哈希---求最小循环节)
  - [字符串哈希 - 枚举因子数求子串的最小循环节](#字符串哈希---枚举因子数求子串的最小循环节)
...
## [字符串哈希 - 求最小循环节](https://ac.nowcoder.com/acm/contest/42400/M)
...
namespace lrq {
    ...
    // 旧版本，暴力枚举长度为 len 的各部分是否相同来判断循环节
    ...
    // }
    // 新版本，利用最大相同前后缀找 s 的最小循环节
    std::string get_loop(ll& len) {
        for (len = 1; len <= (n >> 1); len ++) {
            if (get(1, n - len) == get(len + 1, n)) break;
        }
        if (len > (n >> 1)) len = n;
        return std::string(s.begin() + 1, s.begin() + 1 + len);
    }
...
## [字符串哈希 - 枚举因子数求子串的最小循环节](https://www.luogu.com.cn/problem/P3538)
...
namespace lrq {
    ...
    std::pair<ull, ull> get(ll l, ll r) {
        return {
            H1[r] - H1[l - 1] * P1[r - l + 1],
            H2[r] - H2[l - 1] * P2[r - l + 1]
        };
    }
    // 旧版本，暴力枚举长度为 len 的各部分是否相同来判断循环节
    // std::string get_loop(ll& len) {
    ...
    // 枚举因数求子串的最小循环节
    ll prs[N], prs_on[N], cnt; // euler 筛质数
    ll mi_pdv[N]; // 记录 i 的最小质因子
    void get_primes() {
        cnt = 0;
        mi_pdv[1] = 1;
        for (ll i = 2; i <= n; i ++) {
            if (prs_on[i] == 0) prs[++ cnt] = i, mi_pdv[i] = i;
            for (ll j = 1; prs[j] * i <= n; j ++) {
                prs_on[prs[j] * i] = 1;
                mi_pdv[prs[j] * i] = prs[j];
                if (i % prs[j] == 0) break;
            }
        }
    }
    // // 获取 n 的所有质因数及出现的次数，n <= 1e12
    // // 时间复杂度：n * sqrt(n)
    // std::vector<std::array<ll, 2>> get_pdvs(ll n) {
    //     // 比 sqrt(n) 还要大的质因数只可能存在一个
    //     std::vector<std::array<ll, 2>> res;
    //     ll sqr = std::sqrt(n);
    //     for (ll i = 1; prs[i] <= sqr; i ++) {
    //         if (n % prs[i]) continue;
    //         ll x = prs[i], y = 0;
    //         while (n % prs[i] == 0) {
    //             y ++; n /= prs[i];
    ...
    //     return res;
    // }
    // 获取 n 的所有质因数及出现的次数，n <= 1e6
    // 时间复杂度：nlog(n)
    ll ta[N], cta;
    std::vector<std::pair<ll, ll>> get_pdvs(ll n) {
        cta = 0;
        std::vector<std::pair<ll, ll>> res;
        for (ll i = n; i > 1; i /= mi_pdv[i]) {
            ta[++ cta] = mi_pdv[i];
        }
        res.push_back({ta[1], 1});
        for (ll i = 2; i <= cta; i ++) {
            if (ta[i] == res.back().first) res.back().second ++;
            else res.push_back({ta[i], 1});
        }
        return res;
    }
    // 获取 n 的所有因数，可以通过枚举因数来获取最小循环节长度
    std::vector<ll> dv_a[N], dv_b[N];
    void init_dv() {
        for (ll i = 1; i <= n; i ++) {
            for (ll j = i; i * j <= n; j ++) {
                dv_a[i * j].push_back(i);
                if (i - j) dv_b[i * j].push_back(j);
            }
        }
    }
    std::vector<ll> get_dvs(ll n) {
        std::vector<ll> res;
        res.insert(res.end(), dv_a[n].begin(), dv_a[n].end());
        res.insert(res.end(), dv_b[n].rbegin(), dv_b[n].rend());
        return res;
    }
    // 若最小循环节长度为 x, 则循环的次数为：y = n / x
    // 循环次数 y 的质因子，一定在 n 的质因子内，并且令 y 的质因子为 t，则
    // n/t 一样构成循环节，因为 x | n/t，n/t | n
    // 例如：abcabcabcabc, 质因子为：2, 3
    // 最小循环节为：3, 循环次数为 4
    // 那么 4 的质因子 2 作为循环次数时，循环节长度为 6 也是循环节
    // 罗列出 4 的所有质因子然后压缩倍数就可以得到最小循环节长度
    void get_loop(ll& len, ll l, ll r) {
        if (get(l, r - 1) == get(l + 1, r)) { len = 1; return; }
        cta = 0;
        ll n = r - l + 1;
        len = n;
        // 从小到大获取质因子
        for (ll i = n; i - 1; i /= mi_pdv[i]) ta[++ cta] = mi_pdv[i];
        for (ll i = 1; i <= cta; i ++) {
            if (get(l, r - len / ta[i]) == get(l + len / ta[i], r)) {
                len /= ta[i];
            }
        }
    }
...
循环节的长度：$n - ne[n] > \frac{n}{2}\ ?\ n : n - ne[n]$
...
## [扩展 exkmp](https://www.luogu.com.cn/problem/P5410)
...
namespace lrq {
	...
	void get_ne() {
		ne[1] = n; ne[2] = 0;
		for (ll i = 1, j = 2; j <= n && P[i] == P[j]; i ++, j ++, ne[2] ++);
		ll st = 2, len = ne[2];
		for (ll i = 3; i <= n; i ++) {
			// P[j:ne[j]] = P[st:ne[j]]
			// >> P[i:ne[j]-1] = P[st:ne[j]-1]
			// >> 若 ne[st] < ne[j] - 1
			// >>>> P[i:ne[st]] = P[st:ne[st]]
			// >> 若 ne[st] >= ne[j] - 1
			// >>>> P[i:ne[j]-1] = P[st:ne[j]-1] 并且需要暴力扩展
			// >>>> 若扩展后的 ne[i] > ne[st], st = i, len = ne[i]
			if (len < ne[i - 1] - 1) ne[i] = len;
			else {
				ne[i] = std::max(ne[i - 1] - 1, 0ll);
				for (ll p = i + ne[i], q = 1 + ne[i]; p <= n && P[p] == P[q]; p ++, q ++, ne[i] ++);
				if (ne[i] > len) st = i, len = ne[i];
			}
		}
	}
	void get_ex() {
		...
		for (ll i = 1; i <= m; i ++) {
			if (i > ed) {
				ex[i] = 0;
				for (ll p = i, q = 1; p <= m && q <= n && S[p] == P[q]; p ++, q ++, ex[i] ++);
				st = i, ed = i + ex[i] - 1;
			}
			else {
				if (i + ne[i - st + 1] - 1 < ed) ex[i] = ne[i - st + 1];
				else {
					ex[i] = ed - i + 1;
					for (ll p = i + ex[i], q = 1 + ex[i]; p <= m && q <= n && S[p] == P[q]; p ++, q ++, ex[i] ++);
					st = i; ed = i + ex[i] - 1;
				}
			}
		}
