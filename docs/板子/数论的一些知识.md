...
void solve() {
    std::cin >> n >> m;
    for (long i = 1; i <= m; i ++) std::cin >> a[i];
    for (long i = 1; i < (1 << m); i ++) {
        auto vt = get(i);
        long mo = (vt.size() & 1) ? 1 : -1;
        long t = 1;
        for (auto x : vt) {
            t = lcm(t, x);
            if (t > n) break;
...
long C(long a, long b, long mod) {
    if (a < b) return 0;
    long p = fac[a], q = fac[b] * fac[a - b] % mod;
    long d, x, y; d = exgcd(q, mod, x, y);
    return (p * x % mod + mod) % mod;
}

long lucas(long a, long b, long mod) {
    if (b == 0) return 1;
...
void solve() {
    fac[0] = 1;
    std::cin >> a >> b >> p;
    for (long i = 1; i < N; i ++) fac[i] = fac[i - 1] * i % p;
...
bool ny(long a, long b, long& res) {
...
bool ny(long a, long mod, long& res) {
    long d, x, y;
    d = exgcd(a, mod, x, y);
    if (d != 1) return false;
...
void solve() {
    std::cin >> n;
    for (long i = 1; i <= n; i ++) std::cin >> m[i] >> a[i];
    ...
    for (long i = 2; i <= n; i ++) {
        long a2 = a[i], m2 = m[i];
        long a = m1, b = m2, c = ((a2 - a1) % m2 + m2) % m2;
        long d, x, y;
        d = exgcd(a, b, x, y);
        if (c % d) {
            std::cout << -1 << '\n';
...
// long qmul(long x, long y, long m) {
//     x %= m; y %= m;
//     long d = ((long double)x * y / m);
//     d = x * y - d * m;
//     if (d >= m) d -= m;
//     if (d < 0) d += m;
//     return d;
// } // 32 位内可以试着用
long qmul(__int128 a, __int128 b, __int128 mod) {
    return a % mod * (b % mod) % mod;
}
long qpow(long a,long n,long mod) { //快速幂
    long res=1;
    while(n) {
...
bool MRtest(long n) { //Miller Rabin Test
    if(n<3||n%2==0) return n==2;//特判
    long u=n-1,t=0;
    while(u%2==0) u/=2,++t;
    long ud[]={2,325,9375,28178,450775,9780504,1795265022};
    for(long a:ud) {
        long v=qpow(a,u,n);
...
struct big {
    ...
    std::vector<long> add(const std::vector<long>& a, const std::vector<long>& b) const {
        std::vector<long> res;
    ...
    std::vector<long> sub(const std::vector<long>& a, const std::vector<long>& b) const {
        std::vector<long> res;
        long n = a.size(), m = b.size();
    ...
    std::vector<long> mul(const std::vector<long>& a, const std::vector<long>& b) const {
        long n = a.size(), m = b.size();
        std::vector<long> res(n + m + 10, 0);
        for (long i = 0; i < n; i ++) {
            for (long j = 0; j < m; j ++) {
                res[i + j] += a[i] * b[j];
            }
            for (long _ = i, t = 0; _ < i + m || t; _ ++) {
                t += res[_];
                res[_] = t % mod;
                t /= mod;
            }
    ...
    std::vector<long> div(const std::vector<long>& a, long b) const {
        std::vector<long> res(a.size());
        long n = a.size();
        long t = 0;
    ...
    long bmod(const std::vector<long>& a, long b) const {
        std::vector<long> res(a.size());
        long n = a.size();
        long t = 0;
        for (long i = n - 1; i >= 0; i --) {
    ...
    bool cmp(const std::vector<long>& a, const std::vector<long>& b) const {
        long n = a.size(), m = b.size();
        if (n != m) return n < m;
        for (long i = n - 1; ~i; i --) {
